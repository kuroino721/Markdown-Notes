...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if ((window as any).__TAURI__) {
    try {
      ...
      window.onerror = function (message, source, lineno, colno, error) {
        writeTextFile(
          'JS_CRASH_LOG.txt',
          `UI Crash: ${message} at ${source}:${lineno}:${colno}\n${error?.stack}`
        ).catch(console.error);
        return false;
      };
      window.addEventListener('unhandledrejection', function (event) {
        writeTextFile('JS_PROMISE_CRASH.txt', `Promise crash: ${event.reason}`).catch(
          console.error
        );
      });
    } catch (e) {
  ...
  if ((window as any).__TAURI__) {
    adapter.onFileOpen(() => {
      /* already handled by adapter.onFileOpen */
    });
    ...
    btnSync.addEventListener('click', async (e) => {
      ...
      try {
        ...
        if ((window as any).__TAURI__) {
          const { writeTextFile } = await import('@tauri-apps/plugin-fs');
          await writeTextFile(
            'JS_TRACE_SYNC_ERROR.txt',
            `Caught sync error: ${error.message || error}`
          ).catch(console.error);
        }
