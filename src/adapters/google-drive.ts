...
export async function initGoogleDrive(): Promise<void> {
  ...
  if (isTauri()) {
    ...
    await invoke('frontend_log', {
      level: 'info',
      message: `[SYNC] Initializing G-Drive for Tauri. Client IDs: WEB=${!!CLIENT_ID_WEB}, DESKTOP=${!!CLIENT_ID_DESKTOP}`,
    });
  }
  ...
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      clearInterval(pollScripts);
      if (!gapiInited || !gisInited) {
        console.error('[DEBUG] GoogleDriveService: Google API scripts failed to load in time', {
          gapiInited,
          gisInited,
        });
...
export async function signInGoogleDrive(
  ...
  if (isTauri() && !silent) {
    ...
    await invoke('frontend_log', {
      level: 'info',
      message: '[SYNC] Starting Auth (PKCE) for WSL2/Desktop',
    });
    await invoke('frontend_log', {
      level: 'info',
      message: `[SYNC] redirect_uri set to: ${redirectUri}`,
    });
    ...
    try {
      ...
      await invoke('frontend_log', {
        level: 'info',
        message: `[SYNC] Auth URL generated (client_id: ${CLIENT_ID_DESKTOP?.substring(0, 10)}...)`,
      });

      // 3. Start loopback server and wait for code
      await invoke('frontend_log', {
        level: 'info',
        message: '[SYNC] Starting loopback server on 51737...',
      });
      ...
      await invoke('frontend_log', {
        level: 'info',
        message: '[SYNC] Opening external browser...',
      });
      ...
      await invoke('frontend_log', {
        level: 'info',
        message: '[SYNC] Authorization code received from loopback server',
      });

      // 5. Exchange code for token
      await invoke('frontend_log', {
        level: 'info',
        message: '[SYNC] Exchanging code for token...',
      });
      ...
      await invoke('frontend_log', {
        level: 'info',
        message: `[SYNC] Token exchange status: ${tokenResponse.status}`,
      });
      ...
      if (accessToken) {
        await invoke('frontend_log', {
          level: 'info',
          message: '[SYNC] Token exchange successful (Token received)',
        });
        ...
        await invoke('frontend_log', {
          level: 'error',
          message: `[SYNC] No access token in response: ${JSON.stringify(tokenData).substring(0, 100)}...`,
        });
      ...
      await invoke('frontend_log', {
        level: 'error',
        message: `[SYNC] Auth flow exception: ${e.message || e}`,
      });
      throw e;
    }
  }
